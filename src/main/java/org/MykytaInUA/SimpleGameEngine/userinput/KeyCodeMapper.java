...
/**
 * Provides a mapping between key codes from different input systems (NEWT and
 * Swing) and a unified set of virtual key codes used internally by the
 * application.
 * 
 * <p>This class supports mappings for numeric keys, alphabetic keys, and
 * various control and function keys. It maintains two separate maps for
 * translating key codes from NEWT and Swing input events into the unified key
 * codes.</p>
 */
public class KeyCodeMapper {

    /**
     * A map containing NEWT key codes and their corresponding internal virtual key
     * codes.
     */
    private static final HashMap<Integer, Integer> newtKeyMap = new HashMap<>();

    /**
     * A map containing Swing key codes and their corresponding internal virtual key
     * codes.
     */
    private static final HashMap<Integer, Integer> swingKeyMap = new HashMap<>();

    /**
     * Retrieves the mapped internal virtual key code for a given NEWT key code.
     *
     * @param keyCode the NEWT key code to be mapped.
     * @return the corresponding internal virtual key code, or {@link #VK_UNDEFINED}
    ...
    /**
     * Retrieves the mapped internal virtual key code for a given Swing key code.
     *
     * @param keyCode the Swing key code to be mapped.
     * @return the corresponding internal virtual key code, or {@link #VK_UNDEFINED}
     *         if no mapping exists.
     */
